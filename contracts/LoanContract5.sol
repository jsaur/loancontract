...
/* Introduces the concept of a share price. This prevents someone lending 1 wei and makes distribution
   logic simplier. If a lender lends more than the share price we return to sender. Same if the borrower
   sends more than can be evenly distributed 
*/
contract LoanContract5 {
    //Constructor vars
    ...
    function lend() private {
        ...
        uint amountLent = (msg.value / sharePrice) * sharePrice;
        
        // Handle lender lending twice
        if (lenderAccounts[msg.sender].amountLent == 0) {
            numLenders++;
            lenderAddresses.push(msg.sender);
            lenderAccounts[msg.sender] = LenderAccount(amountLent, 0);
        } else {
            lenderAccounts[msg.sender].amountLent += amountLent;
        }
        
        amountRaised += amountLent;
        LentToLoan(msg.sender, amountLent);
        
        checkLoanFunded();
        
        returnToSender(msg.value - amountLent);
    }
    
    // Send back any extra: note this currently runs out of gas and I can't figure out why
    function returnToSender(uint amount) private {
        if (amount > 0) {
            if (msg.sender.send(amount)) {
                SentBack(msg.sender, amount);
            }
        }
    }
    ...
    function repay() private {
        ...
        // Distribute wei evenly to lenders, if there's a remainder send it back to the borrower
        uint amountDistributed = 0;
        for (uint i = 0; i < lenderAddresses.length; i++) {
            address currentLender = lenderAddresses[i];
            uint amountForLender = (msg.value * lenderAccounts[currentLender].amountLent) / loanAmount; /* Division in solidity throws away the remainder*/
        ...
        amountRepaid += amountDistributed;
        RepaidByBorrower(borrowerAddress, amountDistributed);
        
        checkLoanRepaid();
        
        returnToSender(msg.value - amountDistributed);
    }
